...
def parse_args(args=None):
    parser = argparse.ArgumentParser(description="Object Tracker Configuration")
...
def run_object_tracking(args):
    ...
    print(f"Reading video from {args.video}")
    from glob import glob
    print(f"Searching for file in folder {os.path.dirname(args.video) + '/*'}: {glob(os.path.dirname(args.video) + '/*')}")
    out = None
    ...
    while True:
        ...
        if args.framework == 'tflite':
            ...
            if args.model == 'yolov3' and args.tiny == True:
        ...
        output_json = {"tracks": [], "frame_num": frame_num, "class_names": [], "bboxes": [], "scores": [], "video_path": args.video}
        ...
        # draw bbox on screen
            ...
            cv2.putText(frame, class_name + "-" + str(track.track_id) + "- score: " + str(np.round(last_score, 2)),(int(bbox[0]), int(bbox[1]-10)),0, 0.75, (255,255,255),2)
            
            output_json["tracks"].append(track.track_id)
            output_json["class_names"].append(class_name)
            output_json["bboxes"].append(bbox.tolist())
            output_json["scores"].append(last_score)

        # if enable info flag then print details about each track
            if args.info:
                print("Tracker ID: {}, Class: {},  BBox Coords (xmin, ymin, xmax, ymax): {}".format(str(track.track_id), class_name, (int(bbox[0]), int(bbox[1]), int(bbox[2]), int(bbox[3]))))
        
        # save output json to database
        # print(f"output_json: {output_json}")
        # import request
        
        # send post request to server
        try:
            requests.post("http://flask-app:5000/frames", json=output_json)
...
if __name__ == '__main__':
    ...
    try:
        args = parse_args()
        run_object_tracking(args)  # Pass the argparse.Namespace object
    except SystemExit:
        pass
